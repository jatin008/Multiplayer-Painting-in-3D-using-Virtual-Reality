...
public class MeshLineRenderer : MonoBehaviour
{
    ...
    public void AddPoint(Vector3 point, Vector3 facing)
    {
        if (s != Vector3.zero)
        {
            AddLine(ml, MakeQuad(s, point, facing,lineSize, firstQuad));
            firstQuad = false;
        }
    ...
    Vector3[] MakeQuad(Vector3 s, Vector3 e,Vector3 facing , float w, bool all)
    {
        w = w / 2;
